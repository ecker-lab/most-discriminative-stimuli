...
class ActMaxClustImgBase(ABC):
    ...
    def init_model(self, model):
        ...
        model_shifted_readout = translate_model(model, self.center_locations, self.canvas_size)
    ...
    def max_pred_ac_rot(predictions: torch.Tensor) -> torch.Tensor:
        ...
        if predictions.dim() == 2:
            predictions = torch.max(predictions, dim=0)[0]  # max of predictions across orientations
