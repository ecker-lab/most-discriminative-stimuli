...
class EMALossStopper:
    ...
    def __init__(
        self, patience: int = 100, ema_weight: float = 0.9, verbose: bool = False
    ) -> None:
        self.min_loss = 1e10
    ...
    def update(self, loss: float) -> None:
        if self.ema_loss is None:
            ...
            self.ema_loss = (
                self.ema_weight * self.ema_loss + (1 - self.ema_weight) * loss
            )
        ...
        if self.ema_loss < self.min_loss:
            ...
            self.verbose_print(
                f"Not decreased for {self.not_decreased} steps out of {self.patience} patience steps"
            )
...
class ImgPxIntensityChangeStopper:
    ...
    def __init__(
        self, vmin: float, vmax: float, patience: int = 50, verbose: bool = False
    ) -> None:
        """
        Args:
    ...
    def update(self, img: torch.Tensor) -> None:
        ...
        self.verbose_print(
            "max. abs. img. change", np.round(max_change, 2), "monitor intensity levels"
        )
