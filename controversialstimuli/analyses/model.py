...
def norm_unit_act_imgs(
    ...
    elif type_ == "std-clip" and rep_dataloader is None and stds is None and means is None:
        preds = preds / np.clip(np.std(preds, axis=0, keepdims=True, ddof=1), a_min=CLIP_DIV_PRED_MIN, a_max=None)

    elif type_ == "z-score-clip" and rep_dataloader is None and stds is None and means is None:
        preds = (preds - np.mean(preds, axis=0, keepdims=True)) / np.clip(
            np.std(preds, axis=0, keepdims=True, ddof=1), a_min=CLIP_DIV_PRED_MIN, a_max=None
        )
        print(
            f"clipping {np.sum(np.std(preds, axis=0, keepdims=True, ddof=1) < CLIP_DIV_PRED_MIN)} stds to {CLIP_DIV_PRED_MIN}"
        )

    elif type_ == "z-score-clip-ext" and rep_dataloader is None and stds is not None and means is not None:
        print("preds.shape", preds.shape)
        preds = (preds - means) / np.clip(stds, a_min=CLIP_DIV_PRED_MIN, a_max=None)
        print(f"clipping {np.sum(stds < CLIP_DIV_PRED_MIN)} stds to {CLIP_DIV_PRED_MIN}")

    elif type_ == "z-score-clip-coeff-of-var-ext" and rep_dataloader is None and stds is not None and means is not None:
        denom = stds.copy()
        denom[stds < 0.01 * means] = 0.01 * means[stds < 0.01 * means]
        preds = (preds - means) / denom
        print(f"clipping {np.sum(stds < 0.01 * means)} stds to 0.01 * mean {0.01 * means[stds < 0.01 * means]}")

    elif type_ == "rep-mean_max" and rep_dataloader is not None and stds is not None and means is not None:
        if next(iter(rep_dataloader)).responses.shape[0] != 10:
            warnings.warn("Check if repeats dataloader passed")
        mean_resps = np.stack([batch.responses.mean(0).detach().cpu().numpy() for batch in rep_dataloader])
        max_resp = mean_resps.max(0)
        preds /= max_resp

    else:
        raise ValueError("Invalid comibation of `type_`, `rep_dataloader`, `stds` and `means` arguments passed.")
...
def pred_max_rot(transl_model, canvas_size, imgs, device="cuda"):
    ...
    for img in imgs:
        ...
        preds.append(transl_model(crop_to_canvas(torch.tensor(img).squeeze(0).to(device))).detach().cpu().numpy())
